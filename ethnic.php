...
<nav class="mobile-navigation-menu  has-scrollbar" data-mobile-menu>
  ...
  <div class="menu-bottom">
    ...
    <ul class="menu-social-container">
      ...
      <?php
       $select_query="SELECT *FROM`ethnic`";
       $result_query= mysqli_query($con,$select_query);

       while($row=mysqli_fetch_assoc($result_query)){
        $product_id=$row['product6_id'];
        $product_title=$row['product6_title'];
        $product_description=$row['product6_description'];
        $product_image1=$row['product6_image1'];
        $product_price=$row['product6_price'];
